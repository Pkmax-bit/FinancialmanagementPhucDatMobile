...
import { Search, Plus, ArrowLeft, FileText, Calendar } from 'lucide-react';

export default function QuoteListMockup() {
  const quotes = [
    {
      number: 'QT-2025-001',
      ...
      <div className="flex-1 overflow-auto p-4">
        <div className="space-y-3">
          {quotes.map((quote, index) => {
            const statusColors = getStatusColor(quote.statusColor);
              ...
              <div key={index} className="bg-white rounded-xl p-4 shadow">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <div className="text-slate-900 mb-1">{quote.project}</div>
                      <div className="text-xs text-slate-500">{quote.number}</div>
                    </div>
                  </div>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-sm text-slate-700">{quote.customer}</span>
                  </div>

                  <div className="flex justify-between items-center">
                    <span className="text-xs text-slate-500">Tổng giá trị</span>
                    <span className="text-slate-900">${quote.amount.toLocaleString()}</span>
                  </div>
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-sm text-slate-700">{quote.validUntil}</span>
                  </div>
                </div>
